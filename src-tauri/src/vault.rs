...
mod tests {
    ...
    fn credential_roundtrip_and_remove() {
        ...
        let value = "local-test-placeholder-not-a-real-key";
        set(dir.path(), value).unwrap();
        let stored = fs::read(dir.path().join("credential.bin")).unwrap();
        assert!(!stored.windows(value.len()).any(|s| s == value.as_bytes()));
        assert_eq!(get(dir.path()).unwrap(), value);
        set(dir.path(), "").unwrap();
        assert!(get(dir.path()).is_err());
    }
}
