...
**当前是开发中的 0.1.1，尚未完成两端首版验收。** Windows 已生成测试安装包，Android 已生成 APK 并在模拟器安装启动；完整设备验收进度见 [验证记录](docs/VALIDATION.md)。请使用独立测试目录。
...
- 图片列表显示文件名、页序和识别状态，支持拖入提示。模型返回文章标题、起止块与正文/非正文归属；缺失标题单独补齐。只有正文参与精读分段和总结。
